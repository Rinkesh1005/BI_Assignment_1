...
const ListingPage = ({searchQuery}) => {
  ...
  const filteredByType =
    ...
    return (
      ...
      event.tags.includes(searchQuery)
    );
  });
        ...
        <div className="row">
          {filteredEvents.map((event) => (
            <div className="col-md-4 mb-3" key={event._id}>
              ...
              <h3><Link to={`/details/${event._id}`} className="link-dark link-offset-3-hover link-underline link-underline-opacity-0 link-underline-opacity-75-hover">{event.title}</Link></h3>
            </div>
          ))}
        </div>
      </section>
